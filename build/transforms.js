 ...
 */
export function getFirstDataset(datasets = {}) {
    return datasets[Object.keys(datasets)[0]] || [];
}
/**
 * sets a dates dataset based on a date range
 */
export function _helper_setEmptyDates({ dimension, start_date, end_date, time_zone, defaultObject, datasetsProperties, datasets, field, }) {
 ...
 */
export function _helper_formatDateField({ combined, date_field, dateFormat }) {
...
export function combineDatasetsOnField({ datasets, field, defaultObject = {}, includeEmptyDates = false, dateFormat, date_field, time_zone, dimension, start_date, end_date, } = {}) {
    if (!field)
        throw ReferenceError('In order to combine datasets, a field parameter is required');
    ...
    if (date_field && dateFormat)
        _helper_formatDateField({ combined, date_field, dateFormat });
    return combined;
}
