...
const { performanceValues, prettyTimeStringOutputFormat, timeProperty, dateTimeProperty, featureTimeProperty, durationToDimensionProperty, } = CONSTANTS;
...
function isClosedOnDay(options) {
    const { weekday, dayname, parsedDate, date, operations, zone, } = options;
    ...
    if (!weekday)
        throw new Error('missing weekday');
    if (!parsedDate.weekday)
        throw new Error('missing parsedDate.weekday');
...
function getOpenHour(options) {
    ...
    if (!operations)
        throw new Error(`${entity.name} is missing operation details`);
    if (!operations.launch_date)
        throw new Error(`${entity.name} is missing a launch date`);
    if (!operations.is_24_hours && (!operations.business_hours || Object.keys(operations.business_hours).length < 7))
        throw new Error(`${entity.name} is missing business hours`);
    ...
    if (operations.business_hours) {
        ...
        if (dimension === 'hourly') {
            if (parsedDate.weekday === 1 && isClosedOnDay({ weekday: 1, dayname: 'monday', parsedDate, date, operations, zone, }))
                regular_closed = 1;
            if (parsedDate.weekday === 2 && isClosedOnDay({ weekday: 2, dayname: 'tuesday', parsedDate, date, operations, zone, }))
                regular_closed = 1;
            if (parsedDate.weekday === 3 && isClosedOnDay({ weekday: 3, dayname: 'wednesday', parsedDate, date, operations, zone, }))
                regular_closed = 1;
            if (parsedDate.weekday === 4 && isClosedOnDay({ weekday: 4, dayname: 'thursday', parsedDate, date, operations, zone, }))
                regular_closed = 1;
            if (parsedDate.weekday === 5 && isClosedOnDay({ weekday: 5, dayname: 'friday', parsedDate, date, operations, zone, }))
                regular_closed = 1;
            if (parsedDate.weekday === 6 && isClosedOnDay({ weekday: 6, dayname: 'saturday', parsedDate, date, operations, zone, }))
                regular_closed = 1;
            if (parsedDate.weekday === 7 && isClosedOnDay({ weekday: 7, dayname: 'sunday', parsedDate, date, operations, zone, }))
                regular_closed = 1;
        }
...
function sumPreviousRows(options) {
    ...
    if (this.debug) {
        if (OFFSET < 1)
            throw new RangeError(`Offset must be larger than the default of 1 [property:${property}]`);
        if (index - rows < 1)
            throw new RangeError(`previous index must be greater than 0 [index-rows:${index - rows}]`);
    }
    const sum = this.data
...
class RepetereModel {
    ...
    constructor(parameters = {}, options = {}) {
        this.config = Object.assign({
            ...
            model_category: (parameters.modelDocument && parameters.modelDocument.model_configuration && parameters.modelDocument.model_configuration.model_category) ? parameters.modelDocument.model_configuration.model_category : 'regression',
        }, options);
        this.status = {
            trained: false,
    ...
    addMockData({ use_mock_dates = false, }) {
        if (use_mock_dates && this.use_mock_encoded_data)
        ...
        else if (this.use_mock_encoded_data)
    ...
    removeMockData({ use_mock_dates = false, }) {
        if (use_mock_dates && this.use_mock_encoded_data)
            this.DataSet = removeMockDataToDataSet(this.DataSet, { includeConstants: true, mockEncodedData: this.mockEncodedData, });
        else if (use_mock_dates)
            this.DataSet = removeMockDataToDataSet(this.DataSet, {});
        else if (this.use_mock_encoded_data)
            this.DataSet = removeMockDataToDataSet(this.DataSet, { includeConstants: false, mockEncodedData: this.mockEncodedData, });
    }
    ...
    async validateTimeseriesData(options = {}) {
        ...
        if (raw_prediction_inputs.length) {
            ...
            if (raw_prediction_inputs[0][this.prediction_timeseries_date_feature].valueOf() !== forecastDates[0].valueOf())
                throw new RangeError(`Prediction input dates (${raw_prediction_inputs[0][this.prediction_timeseries_date_feature]} to ${raw_prediction_inputs[raw_prediction_inputs.length - 1][this.prediction_timeseries_date_feature]}) must match forecast date range (${forecastDates[0]} to ${forecastDates[forecastDates.length - 1]})`);
        }
    ...
    async getDataSetProperties(options = {}) {
        ...
        const nextValueFunctions = this.prediction_inputs_next_value_functions.reduce((functionsObject, func) => {
            ...
            Object.defineProperty(functionsObject[func.variable_name], 'name', {
    ...
    async timeseriesForecast(options = {}) {
        ...
        await Promisie.each(forecastDates, 1, async (forecastDate) => {
            ...
            if (predictionInput) {
                ...
                const forecast = Object.assign({}, datasetScaledObject, scaledRawInputObject, newPredictionObject, parsedLocalDate, {
    ...
    async predictModel(options = {}) {
            ...
            */
            this.prediction_inputs = raw_prediction_inputs.map(prediction_value => {
                const pred_val = this.use_empty_objects
                    ? Object.assign({}, this.emptyObject, flatten(prediction_value, { maxDepth: 2, delimiter: flattenDelimiter, }))
                    : prediction_value;
        ...
        if (descalePredictions) {
            ...
            predictions = predictions.map((val, i) => {
                ...
                const descaled = Object.assign({}, transformed, empty, (includeInputs && this.config.model_category !== 'timeseries') ? unscaledInputs[i] : {}, { __evaluation, });
                return descaled;
            });
        }
