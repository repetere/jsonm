...
// { num: 1, some: 'val', double_num: 2 } ]
 */
export async function ReduceDataset(reducer) {
    const { name, } = reducer;
    const context = {
        context: {
...
//}
 */
export async function ResolveDataset(datasets = []) {
    let i = 0;
    let datasetData = {};
    if (datasets.length) {
 ...
 */
export function getFunctionString(reductionFunctionData) {
    ...
    if (typeof reducer_function === 'function') {
        ...
        Object.defineProperty(reduceFunction, 'name', {
